		...
		<div id="header" class="page-banner d-flex align-items-center">
			<div class="container">
				<div class="row">
					<div class="col-12">
						<div class="page-head">
							<h2 class="page-heading">Here's <del>Johnny</del> {{ $user->nickname }} !!</h2>
						</div>
					</div>
				...
				<div class="col-lg-8">
					<div class="blog-content">
						<div class="blog-photo" style="width: 50%; margin-top: -110px;">
							<img src="../images/virgin.png" alt="blog-large">
						</div>
						<form action="/donate" method="POST" class="login-form">
							@method('PUT')
							@csrf
							<div class="input-item">
            		<input type="text" name="nickname" placeholder="{{ $user->nickname }}" class="input-line-simple" disabled style="color:aliceblue">
	            </div>

	            <div class="input-item">
	                <input type="text" name="email" placeholder="{{ $user->email }}" class="input-line-simple" disabled style="color:aliceblue">
	            </div>
	      		</form>
				</div><!-- .col -->
			...
			<div class="col-lg-4">
					...
					<div class="sidebar-section">
						<div class="sidebar-widget category">
							...
							<ul>
								@if ($user->nickname != session('user')->nickname)
            						@if(!empty($follow))
										<li><a href="{{ url('/unFollowUser/'.$user->id) }}">Unfollow {{ $user->nickname }}</a></li>
									@else
										<li><a href="{{ url('/followUser/'.$user->id) }}">Follow {{ $user->nickname }}</a></li>
            						@endif
								@endif
							</ul>
						</div><!-- .sidebar-widget -->
